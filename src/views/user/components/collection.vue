...
<template lang="">
    <div class="bg-slate-100 h-screen w-full">
        <!-- 没有内容提示 -->
        <div class="w-full" v-show="noteList.length == 0">
            <div class="flex justify-center flex-col items-center pt-6">
                ...
                <span class="py-2 text-gray-600">暂无收藏哦！</span>
                <button class=" mt-6 border border-gray-400 text-black px-6 py-2 rounded-2xl text-sm">去探索</button>
            </div>
        </div>
        <!-- 有内容进行展示 -->
        <div class="w-full" v-show="noteList.length > 0">
            <div class="w-full p-2" v-show="noteList.length > 0">
...
let noteList = ref<blogSharesTable[]>([] as blogSharesTable[])
...
const props = defineProps({
    ...
    hidden: { //是否是本人看自己的 --别人也可以看(本人可以选择加密)
        type: Boolean,
        default: false,
    }
});
...
const initData = () => {
    ...
    if (res != null) {
        ...
        if (curUser.collectList == null) {
            return;
        }
        db.getNotesByIds(curUser.collectList!.map(String)).then((res) => {
            noteList.value = res;
        })
    }
}
