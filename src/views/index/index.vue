<template lang="">
    <div class="w-screen h-screen bg-slate-100">
        ...
        <header id="homeTabs" ref="header">
            <div class=" py-2 bg-white relative w-full flex border-b border-gray-100 items-center px-2"> 
                ...
                <div  class="grow flex items-center justify-center gap-6"> 
                    <span @click="navigation('findFollowPage')" class="text-gray-500 hover:text-black hover:font-bold" @touchstart="changeMode(0, 'findFollowPage')" >关注</span>
                    <span :class="{ 'text-black font-bold': defaultHover }" class=" text-gray-500  hover:text-black hover:font-bold" @touchstart="changeMode(3, 'findHomePage')">发现</span>
                    <!-- <span class="text-gray-500 hover:text-black hover:font-bold" @touchstart="changeMode(2, 'findLocalPage')" >附近</span> -->
                </div>
        ...
        <div class="w-full bg-white overflow-y-auto dark:bg-slate-900" style="height:calc(100vh - 100px)"> 
...
const changeMode = (type:number, goPathName: string) => {
    if (type == 0 || type == 3) defaultHover.value = false
    else defaultHover.value = true
    navigation(goPathName)
}
</script>
<style lang="scss">
