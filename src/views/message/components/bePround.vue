<!-- 收到的赞和收藏 -->
<template lang="">
    <div class="w-full" v-if="dataList.length == 0">
        <div class="w-full mt-6">
            <div class="flex justify-center flex-col items-center pt-6">
                ...
                <span class="py-2 text-gray-600">暂无赞美和收藏哦，多发笔记可以收获赞和收藏</span>
                <button class=" mt-2 border border-gray-400 text-black px-6 py-2 rounded-2xl text-sm">去分享</button>
            </div>
        </div>
    </div>
    ...
    <div v-if="dataList.length > 0">
        <div v-for="(item, index) in dataList">
            <itemListCom :userId="item.id" :username="item.username"
              :title="item.title"
              :headerImageId="item.headerImg"
              :type="item.type"
             />
        </div>
    </div>
...
import itemListCom from "./listItem.vue"
...
let noteMask: { [key: string]: blogSharesTable } = reactive({}) //noteID和note的映射表
let dataList = ref<(User & {title: string} & {noteId: string} & {type:'like' | 'mark'})[]>([]); //列表：noteId, User
...
const initData = () =>{
    ...
    if (res?.code != -1) {
        ...
        if (curUser.beProudCon || curUser.beProudLike) {
            if (curUser.beProudLike) {
                curUser.beProudLike.forEach((_value, key:any) => {
                    console.log(key);
            ...
            if (curUser.beProudCon) {
                curUser.beProudCon.forEach((_value, key:any) => {
                    console.log(key);
                   if (!idList.includes(key)) {
                       idList.push(key)
                   }
        ...
        //形成一个笔记的map表
        if (idList.length > 0) {
            db.getNotesByIds(idList).then(res => {
                let index = 0;
                idList.forEach((id) => {
                    noteMask[id] = res[index++]
                });

                //第二部计算出每一列的笔记
                if (curUser.beProudLike) {
                    curUser.beProudLike.forEach((value, key:any) => {
                        value.forEach((id: any) => {
                            let user = userDb.getUserById(parseInt(id))
                            console.log(noteMask[key]);
                            
                            dataList.value.push({ noteId: key, title:noteMask[key].title, ...user!, type: 'like'});
                        });
                    });
                }

                if (curUser.beProudCon) {
                    curUser.beProudCon.forEach((value, key:any) => {
                        value.forEach((id: any) => {
                            let user = userDb.getUserById(parseInt(id))
                            dataList.value.push({ noteId: key, title:noteMask[key].title, ...user!, type: 'mark'});
                        });
                    });
                }
            })
