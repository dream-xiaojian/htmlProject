<template lang="">
    <div class="w-full">
        <div class="w-full mt-6" v-show="dataList.length == 0">
            <div class="flex justify-center flex-col items-center pt-6">
                ...
                <span class="py-2 text-gray-600">还没有被粉丝关注过哦</span>
...
let dataList = ref<UserType[]>([] as UserType[]); //列表：userId
...
const initData = () =>{
    ...
    if (res?.code != -1) {
        ...
        let forEachList = curUser.fansList ?? []
        forEachList.forEach((item: number) => {
            let user = userDb.getUserById(item) as UserType
            if (user != null) {
                user.type = 'follow'
